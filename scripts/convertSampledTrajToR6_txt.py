...
def _get_filenames_and_classes(dataset_dir):
    ...
    # with open('../sampled_relative_test1.csv') as csvfile:
    #
    #     count = 0
    ...
    with open('../sampled_relative.txt') as csvfile :
        for line in csvfile.readlines():
            row = line.strip().split(' ')
            trajectory_abs.append(row)
            
    print('Total data: '+ str(len(trajectory_abs)))
    # for temp in trajectory_abs :
        # print(temp)
    # print("*"*40)
    ...
    for i in range(len(trajectory_abs)-1):
        ...
        relative_pose = [X, Y, Z, relative_rot[0], relative_rot[1], relative_rot[2], relative_rot[3]]
        if any(relative_pose) is None :
            print(len(relative_pose))
            print(relative_pose)
            print(i)
        relative_pose.insert(0,"ef")
    ...
    with open('../out1.csv', 'w+') as f:
        ...
        for i in range(len(trajectory_relative)):
            ...
            tmpStr = str(trajectory_abs[i][0]) + ',' + r1 + ',' + r2 + ',' + r3 + ',' + r4 + ',' + r5 + ',' + r6
            print("i", tmpStr)
            f.write(tmpStr + '\n')        
...
def main():
    _get_filenames_and_classes('/home/mongsil/workspace/datasets/EuRoC/V1_01_easy')
